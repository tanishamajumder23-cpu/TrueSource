/**
 * Veristate extension — background service worker (Manifest V3).
 *
 * Two responsibilities:
 *   1. Own the right-click "Fact-check selection" menu item.
 *   2. Be the ONLY thing that talks to the Veristate API.
 *
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({
    id: 'veristate-check-selection',
    title: 'Fact-check "%s" with Veristate',
    contexts: ['selection'],
  });
});

chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'veristate-check-selection' || !tab?.id) return;

  const text = (info.selectionText || '').trim();
  if (text.length < 10) {
    notifyTab(tab.id, { type: 'VERISTATE_ERROR', message: 'Select a bit more text — at least a full sentence.' });
    return;
  }
  ...
  notifyTab(tab.id, { type: 'VERISTATE_LOADING', query: text });

  try {
    const results = await analyze(text);
    notifyTab(tab.id, { type: 'VERISTATE_RESULTS', ...results });
  } catch (error) {
    notifyTab(tab.id, { type: 'VERISTATE_ERROR', message: error.message });
  }
});
...
async function analyze(text) {
  ...
  try {
    ...
    throw new Error(`Can't reach Veristate at ${apiUrl}. Is the backend running?`);
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === 'VERISTATE_ANALYZE') {
  ...
  if (message?.type === 'VERISTATE_GET_API_URL') {
