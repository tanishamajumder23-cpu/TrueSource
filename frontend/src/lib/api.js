...
async function requestJson(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`Cannot reach the TruthSource API at ${API_URL}. Is the backend running?`);
  }
...
export function analyzeVideoStream(file, { onEvent, onUploadProgress, signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener('error', () => reject(new Error(`Cannot reach the TruthSource API at ${API_URL}.`)));
